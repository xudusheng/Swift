...
//  XDSSideMenu.m
//
//  Created by Himly on 15/8/25.
//  Copyright (c) 2015年 Cactus. All rights reserved.
//

#import "XDSSideMenu.h"

@implementation UIViewController (XDSSideMenu)

- (XDSSideMenu *)sideMenuViewController
{
    UIViewController *iter = self.parentViewController;
    while (iter) {
        if ([iter isKindOfClass:[XDSSideMenu class]]) {
            return (XDSSideMenu *)iter;
...
//
        ...
        UISwipeGestureRecognizer *swipeOpenGestureRecognizer = [[UISwipeGestureRecognizer alloc] initWithTarget:self action:@selector(swipeGestureRecognized:)];
        swipeOpenGestureRecognizer.direction = UISwipeGestureRecognizerDirectionLeft;
        swipeOpenGestureRecognizer.delegate = self;
        swipeOpenGestureRecognizer.cancelsTouchesInView = NO;
        swipeOpenGestureRecognizer.delaysTouchesBegan = YES;
        [self.view addGestureRecognizer: swipeOpenGestureRecognizer];
        self.swipeOpenGestureRecognizer = swipeOpenGestureRecognizer;
...
- (void)__presentMenuViewContainerWithMenuViewController:(UIViewController *)menuViewController
{
    ...
    if ([self.delegate conformsToProtocol:@protocol(XDSSideMenuDelegate)] && [self.delegate respondsToSelector:@selector(sideMenu:willShowMenuViewController:)]) {
...
- (void)__showMenuViewController:(UIViewController *)menuViewController
{
    ...
    [UIView animateWithDuration:self.animationDuration animations:^{
        ...
        if (!self.visible && [self.delegate conformsToProtocol:@protocol(XDSSideMenuDelegate)] && [self.delegate respondsToSelector:@selector(sideMenu:didShowMenuViewController:)]) {
...
/*
 ...
 if (!self.rightMenuVisible && [self.delegate conformsToProtocol:@protocol(XDSSideMenuDelegate)] && [self.delegate respondsToSelector:@selector(sideMenu:didShowMenuViewController:)]) {
...
- (void)__hideMenuViewControllerAnimated:(BOOL)animated
{
    ...
    if ([self.delegate conformsToProtocol:@protocol(XDSSideMenuDelegate)] && [self.delegate respondsToSelector:@selector(sideMenu:willHideMenuViewController:)]) {
    ...
    void (^completionBlock)(void) = ^{
        ...
        if (!strongSelf.visible && [strongSelf.delegate conformsToProtocol:@protocol(XDSSideMenuDelegate)] && [strongSelf.delegate respondsToSelector:@selector(sideMenu:didHideMenuViewController:)]) {
...
- (BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldReceiveTouch:(UITouch *)touch
{
    ...
    return YES;
/*
    if (self.panFromEdge && [gestureRecognizer isKindOfClass:[UISwipeGestureRecognizer class]] && !self.visible) {
...
//        case UIGestureRecognizerStateEnded:
            if ([self.delegate conformsToProtocol:@protocol(XDSSideMenuDelegate)] && [self.delegate respondsToSelector:@selector(sideMenu:swipeGestureRecognized:)]) {
                [self.delegate sideMenu:self swipeGestureRecognized:swipeGesture];
...
{
    if ([self.delegate conformsToProtocol:@protocol(XDSSideMenuDelegate)] && [self.delegate respondsToSelector:@selector(sideMenu:didRecognizePanGesture:)])
...
//    NSLog(@"cactus________%@",NSStringFromCGPoint(point));
...
//            self.contentViewContainer.center = CGPointMake((self.contentViewInLandscapeOffsetCenterX + [self __viewGetWidth]) , self.contentViewContainer.center.y);
            ...
            if (self.contentViewContainer.frame.origin.x >= (self.contentViewInLandscapeOffsetCenterX + [self __viewGetWidth]/2) )
        ...
        //
        if (point.x < 0) {
            point.x = MAX(point.x, -[UIScreen mainScreen].bounds.size.width);
        } else {
            point.x = MIN(point.x, [UIScreen mainScreen].bounds.size.width);
        }
//        NSLog(@"cactus____%@",NSStringFromCGPoint(point));

        [recognizer setTranslation:point inView:self.view];
        
        if (!self.didNotifyDelegate) {
            if (point.x > 0) {
                if (!self.visible && [self.delegate conformsToProtocol:@protocol(XDSSideMenuDelegate)] && [self.delegate respondsToSelector:@selector(sideMenu:willShowMenuViewController:)]) {
...
-(void)pressesBegan:(NSSet<UIPress *> *)presses withEvent:(UIPressesEvent *)event{
    ...
    if ( presses.anyObject.type == UIPressTypeMenu) {
        ...
        if (self.visible == NO){
            ...
            if ([self.delegate conformsToProtocol:@protocol(XDSSideMenuDelegate)] && [self.delegate respondsToSelector:@selector(sideMenu:pressesBegan:withEvent:)]) {
...
@implementation XDSSideMenu (Animation)
- (void)flashMenu
{
    // Animate to deappear
    __typeof(&*self) __weak weakSelf = self;
